...
Output directory layout:
  ...
  {output_dir}/plots/stripes/                — Chapter-4 heatmap stripe + comparison figures
  {output_dir}/plots/ffpp_*.png              — FF++ detection visualizations

Detection visualisations saved: ROC, precision-recall, confusion matrix (raw and
normalised), and score-distribution PNGs. Intrinsic explanation quality is summarised
by temporal SSIM and deletion/insertion AUC.

Notes:
  - Checkpoint loading uses weights_only=False (PyTorch 2.6+).
  - Video reading falls back gracefully when a video path is unavailable.
  - Heatmap videos use an annotated overlay with verdict text and a contour of the
...
def run_evaluation(config: EAHNConfig, breakdown_by_manipulation: bool = False):
    ...
    try:
        ...
        import traceback
        print("\n" + "!" * 70)
        print("[Eval] WARNING: deletion/insertion AUC FAILED and was left at 0.0/0.0.")
        print(f"  Reason: {type(e).__name__}: {e}")
        print("  These zeros are NOT a real result. Fix the cause before trusting "
              "the explanation metrics.")
        traceback.print_exc()
        print("!" * 70 + "\n")
    ...
    # ── Chapter-4 heatmap stripe figures ──────────────────────────────────────
    # Assemble the per-frame XAI overlays (written by save_xai_overlays above)
    # into the multi-frame stripe panels and the real-vs-fake comparison panels
    # used as figures in Chapter 4.
    try:
        from scripts.build_heatmap_stripes import build_heatmap_stripes
        _overlay_dir = _PPath(config.output_dir) / "plots" / "heatmaps"
        _stripe_dir  = _PPath(config.output_dir) / "plots" / "stripes"
        build_heatmap_stripes(
            overlay_dir=_overlay_dir,
            out_dir=_stripe_dir,
            manipulation=getattr(config, "active_manipulation", ""),
        )
    except Exception as _stripe_err:
        print(f"[HeatmapStripes] skipped: {_stripe_err}")
