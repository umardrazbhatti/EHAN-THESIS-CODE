...
class GradCAMExplainer:
    def __init__(self, eahn_model, target_layer: nn.Module):
        ...
        # After classifier-surgery the EAHN classifier may take a wider input
        # (concatenated pooled features) than d_model, so its weight has shape
        # (1, in_features) with in_features >= d_model. We copy only the first
        # d_model columns into this spatial-only probe head. When the dimensions
        # already match (in_features == d_model) the slice is a no-op.
        _d = eahn_model.config.d_model
        self.spatial_model.classifier.weight.data.copy_(
            eahn_model.classifier.weight.data[:, :_d]
        )
