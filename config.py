...
class EAHNConfig:
    ...
    # ── Loss weights ──────────────────────────────────────────────────────────
    lambda1: float = 2.0       # L_exp weight when masks ARE available (supervised MSE, scale 0-1)
    lambda1_weak: float = 0.1  # L_exp weight when masks are NOT available (weak prior: entropy+TV, scale 1-5)
    lambda2: float = 1.0   # L_temp weight (Fix 3: raised 0.2->1.0 to compensate for cosine-distance scale ~5-10x smaller than old L2)
    lambda_div: float = 0.5    # Phase 20 Fix 2: inter-sample diversity loss weight
    lambda_faith: float = 0.3  # Phase 20 Fix 3: gradient-faithfulness loss weight
    use_faithfulness_loss: bool = True  # Phase 20: enable gradient-faithfulness auxiliary loss
    ...
    # ── Training ──────────────────────────────────────────────────────────────
    epochs: int = 20           # Fix 1: default 20 for specialist runs (was 50)
    ...
    reset_optimizer: bool = False  # if True, skip loading optimizer/scheduler state on resume (use after classifier surgery)
    ...
    # ── Early stopping (Task 3.3 / Fix 1) ────────────────────────────────────
    use_early_stopping:   bool  = False                     # Fix 1: OFF by default — run all epochs
    early_stop_patience:  int   = 5                         # epochs without improvement before halt (only used when use_early_stopping=True)
    ...
    # ── Cross-attention (Phase 20 Fix 1 / legacy Fix 2) ─────────────────────
    cross_attention_mode: str = "content_adaptive"  # Phase 20: "content_adaptive" | "cls" | "legacy"
    attention_temperature: float = 1.0              # Phase 20: sharper attention (was 2.0 in CLS path)
    # Legacy flags kept for backward-compat with Phase 19 checkpoints:
    cross_attention_use_cls_query:   bool  = True   # Fix 2 (Phase 19): CLS-driven path
    cross_attention_temperature:     float = 2.0    # Fix 2 (Phase 19): CLS path temperature

    # ── Explanation warmup schedule (Phase 20 Fix 4) ─────────────────────────
    explanation_warmup_epochs: int = 5  # epochs with lambda1=lambda_div=lambda_faith=0
    explanation_ramp_epochs: int   = 5  # epochs to linearly ramp explanation losses to full weight
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--lambda1_weak", type=float, default=None,
                        help="L_exp weight for weak supervision (no masks, entropy+TV branch). Default 0.1.")
    ...
    parser.add_argument("--reset_optimizer", dest="reset_optimizer",
                        action="store_true", default=None,
                        help="Skip loading optimizer/scheduler state on resume. "
                             "Use after classifier surgery to avoid stale AdamW moments.")
    ...
    parser.add_argument("--no_explanation_suite", dest="explanation_suite",
                        action="store_false")
    parser.add_argument("--use_early_stopping", dest="use_early_stopping",
                        action="store_true", default=None,
                        help="Enable early stopping (default OFF — run all epochs).")
    parser.add_argument("--cross_attention_temperature", type=float, default=None,
                        help="Softmax temperature for CLS cross-attention path (Fix 2 legacy).")
    parser.add_argument("--no_cls_query", dest="cross_attention_use_cls_query",
                        action="store_false",
                        help="Fall back to legacy Q->S cross-attention (Phase 19 safety net).")
    # Phase 20 new args:
    parser.add_argument("--cross_attention_mode", type=str, default=None,
                        choices=["content_adaptive", "cls", "legacy"],
                        help="Cross-attention mode (Phase 20): content_adaptive (default) | cls | legacy.")
    parser.add_argument("--attention_temperature", type=float, default=None,
                        help="Attention temperature for content_adaptive path (default 1.0).")
    parser.add_argument("--lambda_div", type=float, default=None,
                        help="Inter-sample diversity loss weight (Phase 20, default 0.5).")
    parser.add_argument("--lambda_faith", type=float, default=None,
                        help="Gradient-faithfulness loss weight (Phase 20, default 0.3).")
    parser.add_argument("--use_faithfulness_loss", dest="use_faithfulness_loss",
                        action="store_true", default=None,
                        help="Enable gradient-faithfulness auxiliary loss (Phase 20, default ON).")
    parser.add_argument("--no_faithfulness_loss", dest="use_faithfulness_loss",
                        action="store_false")
    parser.add_argument("--explanation_warmup_epochs", type=int, default=None,
                        help="Epochs with explanation losses zeroed (warmup, default 5).")
    parser.add_argument("--explanation_ramp_epochs", type=int, default=None,
                        help="Epochs to linearly ramp explanation losses after warmup (default 5).")
